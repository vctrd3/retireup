...
onSliderChange = value => {
  this.setState({
    value,
    data: Data.slice(value[0],value[1])
  });
};
...
render() {
  return (
    <div>
    <div style={style}>
      <Range min={0} max={Data.length} reverse={true} allowCross={false} value={this.state.value} 
      onChange={this.onSliderChange} 
      />
    </div>
    <table>
      <thead>
        <tr>S&P 500 Total Returns by Year</tr>
      </thead>
      <tbody>
        <tr>
          <th>Year</th>
    ...
    {this.state.data.map(line => {
      return line.totalReturn.includes('-') ? (
        (
          <tr>
            <td>{line.year}</td>
            <td className="red">{line.totalReturn}</td>
          </tr>
        )
      ) :((
        <tr>
          <td>{line.year}</td>
          <td>{line.totalReturn}</td>
        </tr>
      ))
